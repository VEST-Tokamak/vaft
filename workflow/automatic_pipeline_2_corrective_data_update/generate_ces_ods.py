#!/usr/bin/env python3
"""Store one shot's charge-exchange upload as a stage product.

CES is an externally produced diagnostic: a `.mat` file is uploaded by hand
...
from vaft.omas.vest_upstream import build_ces_ods, write_stage_product

LOGGER = logging.getLogger("vaft.generate_ces_ods")


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--shot", required=True, type=int, help="VEST shot number.")
    parser.add_argument("--output", required=True, type=Path, help="Output CES ODS JSON.")
    ...
    parser.add_argument(
        "--options",
        default="ces",
        help="Which external layout to read: `ces` for CES_{shot}.mat, `ids` for IDS_{shot}.mat.",
    )
    ...
    try:
        ...
        LOGGER.exception("CES stage failed for shot %s; recording and continuing", args.shot)
    ...
    LOGGER.info("CES ODS saved to %s (status=%s)", args.output, manifest["status"])
    return 0
