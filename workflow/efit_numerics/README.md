...
ODS (R 0.104–0.760 m at the time; 0.105–0.761 m since #965, Z ±1.185 m) and the grid, and sets `aminor_max` to half
...
## The Picard trajectory as an object (#1038)

The studies above each read EFIT's log for their own purpose. The per-step
trajectory is now a library object: `EFITConfig(iteration_history="summary")`
writes `efit_iteration_history.json` beside a run's outputs, and
`vaft.code.efit.iteration_history_from_workdir` reads the same history out of
any finished run. `notebooks/convergence_study_of_efit.ipynb` is the standard
way to look at one.

The printed `it=` is EFIT's cumulative `nitera`, not an outer-iteration count:
with `NXITER=3` a 39915 slice prints `it=` up to 15. `MXITER` caps the *outer*
loop, which runs `MXITER + 1` passes of one to `NXITER` steps each, the last of
exactly one. So a capped slice has between `MXITER + 1` and
`NXITER * MXITER + 1` steps and an uncapped one at most `NXITER * MXITER`:

- at `NXITER=1` the count decides (the scan's exhausted slices have exactly
  101 steps at `MXITER=100` and 26 at `MXITER=25`);
- at `NXITER>1` it often cannot, and `efit_quality`'s `hit_cap` now says
  `None` there instead of `iterations >= MXITER`, which called 100 steps a cap.

What does decide it is EFIT's own line, `not converged, reached max
iterations`. The log parser used to call every slice without an `iconvr=2`
line `iterations_exhausted`. But `fit.F90` has one silent exit: an inner
loop's first step drops the increment below `ERROR` with the chi-square at or
below `SAIMIN` (`go to 2020`). In the #171 scan that is 22 of the 39
"exhausted" `NXITER=3` slices (39915 @ 317 ms: 145 steps, increment 9.6e-6;
one took it on the very last pass, at 301 steps).
Those are now `no_exit_message`, and `convergence_study.converged()` counts
them as stops on EFIT's criterion. The #924 finding that `NXITER>1` loses
slices was read through the old label and wants re-checking.

The two histories the notebook reads are regenerated by
...
which runs the packaged sample's own k-files at `NXITER=1` and `NXITER=3` and
writes `vaft/data/efit/iteration_history/39915_nxiter{1,3}.json`.
