...
def extract_efit_data(efit_set):
    ...
    data = {
        ...
        'time': efit_set['time'],
        'ip': round(ods['equilibrium.time_slice.0.global_quantities.ip'] / 1e6, 2),  # MA
        ...
        # 'stored_energy': round(ods['equilibrium.time_slice.0.global_quantities.energy_mhd'] / 1e3, 2),  # kJ
    ...
    print("\nPlasma Parameters:")
    print(f"  Plasma Current: {data['ip']} MA")
    ...
    # print(f"  Internal Inductance: {data['internal_inductance']}")
    # print(f"  Stored Energy: {data['stored_energy']} kJ")
    print(f"  Torodial B-field at Axis: {data['b_field_tor_axis']} T")
    print("\nGeometric Parameters:")
    print(f"  Plasma Volume: {data['plasma_volume']} m^3")
    print(f"  Plasma Surface: {data['plasma_surface']} m^2")
...
def generate_efit_history_excel(base_path=None):
    ...
    if all_data:
        ...
        print("Processing complete! Saved to efit_history.xlsx")
    else:
