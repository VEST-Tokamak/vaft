---
title: "Process reference: transients"
author: VEST team
date: 2026-09-18 09:00
category: guide
layout: post
permalink: /reference/process/transients/
guide:
  architecture: Generated per-function reference for vaft.process.transients, read from the standardized docstrings (issue 252).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "transients" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "transients" %}

This page is generated from the docstrings of
[`vaft/process/transients.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/transients.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("transients.<name>")` prints.
