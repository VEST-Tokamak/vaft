---
title: "Process reference: soft_x_rays"
author: VEST team
date: 2026-09-03 10:06
category: guide
layout: post
permalink: /reference/process/soft_x_rays/
guide:
  architecture: Generated per-function reference for vaft.process.soft_x_rays, read from the standardized docstrings (issue 252).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "soft_x_rays" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "soft_x_rays" %}

This page is generated from the docstrings of
[`vaft/process/soft_x_rays.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/soft_x_rays.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("soft_x_rays.<name>")` prints.
