---
title: "Formula reference: transformer"
author: VEST team
date: 2026-09-17 10:00
category: guide
layout: post
permalink: /reference/formula/transformer/
guide:
  architecture: Generated per-function reference for vaft.formula.transformer, read from the standardized docstrings (issue 248).
  prerequisites: None.
...
{% assign category = site.data.formula_catalog.categories | where: "name", "transformer" | first %}
{% assign entries = site.data.formula_catalog.formulas | where: "category", "transformer" %}

This page is generated from the docstrings of
[`vaft/formula/transformer.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/formula/transformer.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.formula.describe("transformer.<name>")` prints.
