---
title: "Process reference: impa"
author: VEST team
date: 2026-09-03 10:09
category: guide
layout: post
permalink: /reference/process/impa/
guide:
  architecture: Generated per-function reference for vaft.process.impa, read from the standardized docstrings (issue 252).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "impa" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "impa" %}

This page is generated from the docstrings of
[`vaft/process/impa.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/impa.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("impa.<name>")` prints.
