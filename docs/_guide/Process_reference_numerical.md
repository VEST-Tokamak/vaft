---
title: "Process reference: numerical"
author: VEST team
date: 2026-09-03 10:02
category: guide
layout: post
permalink: /reference/process/numerical/
guide:
  architecture: Generated per-function reference for vaft.process.numerical, read from the standardized docstrings (issue 252).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "numerical" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "numerical" %}

This page is generated from the docstrings of
[`vaft/process/numerical.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/numerical.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("numerical.<name>")` prints.
