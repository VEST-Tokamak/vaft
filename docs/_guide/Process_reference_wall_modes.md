...
guide:
  architecture: Generated per-function reference for vaft.process.wall_modes, read from the standardized docstrings (issue 252).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "wall_modes" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "wall_modes" %}

This page is generated from the docstrings of
[`vaft/process/wall_modes.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/wall_modes.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("wall_modes.<name>")` prints.
