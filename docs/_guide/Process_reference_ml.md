---
title: "Process reference: ml"
author: VEST team
date: 2026-09-17 12:00
category: guide
layout: post
permalink: /reference/process/ml/
guide:
  architecture: Generated per-function reference for vaft.process.ml, read from the standardized docstrings (issue 252).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "ml" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "ml" %}

This page is generated from the docstrings of
[`vaft/process/ml/`](https://github.com/VEST-Tokamak/vaft/tree/develop/vaft/process/ml):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("ml.<name>")` prints.
