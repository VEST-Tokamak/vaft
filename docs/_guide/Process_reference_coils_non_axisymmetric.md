...
guide:
  architecture: Generated per-function reference for vaft.process.coils_non_axisymmetric, read from the standardized docstrings (issue 252).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "coils_non_axisymmetric" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "coils_non_axisymmetric" %}

This page is generated from the docstrings of
[`vaft/process/coils_non_axisymmetric.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/coils_non_axisymmetric.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("coils_non_axisymmetric.<name>")` prints.
