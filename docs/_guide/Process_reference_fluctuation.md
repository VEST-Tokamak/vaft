---
title: "Process reference: fluctuation"
author: VEST team
date: 2026-09-03 10:12
category: guide
layout: post
permalink: /reference/process/fluctuation/
guide:
  architecture: Generated per-function reference for vaft.process.fluctuation, read from the standardized docstrings (issue 252).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "fluctuation" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "fluctuation" %}

This page is generated from the docstrings of
[`vaft/process/fluctuation.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/fluctuation.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("fluctuation.<name>")` prints.
