---
title: "Process reference: electromagnetics"
author: VEST team
date: 2026-09-03 10:11
category: guide
layout: post
permalink: /reference/process/electromagnetics/
guide:
  architecture: Generated per-function reference for vaft.process.electromagnetics, read from the standardized docstrings (issue 252).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "electromagnetics" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "electromagnetics" %}

This page is generated from the docstrings of
[`vaft/process/electromagnetics.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/electromagnetics.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("electromagnetics.<name>")` prints.
