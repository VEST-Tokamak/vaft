---
title: "Process reference: equilibrium"
author: VEST team
date: 2026-09-03 10:05
category: guide
layout: post
permalink: /reference/process/equilibrium/
guide:
  architecture: Generated per-function reference for vaft.process.equilibrium, read from the standardized docstrings (issue 252).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "equilibrium" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "equilibrium" %}

This page is generated from the docstrings of
[`vaft/process/equilibrium.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/equilibrium.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("equilibrium.<name>")` prints.
