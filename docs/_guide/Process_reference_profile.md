---
title: "Process reference: profile"
author: VEST team
date: 2026-09-03 10:04
category: guide
layout: post
permalink: /reference/process/profile/
guide:
  architecture: Generated per-function reference for vaft.process.profile, read from the standardized docstrings (issue 252).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "profile" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "profile" %}

This page is generated from the docstrings of
[`vaft/process/profile.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/profile.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("profile.<name>")` prints.
