...
guide:
  architecture: Generated per-function reference for vaft.formula.startup, read from the standardized docstrings (issue 248).
  prerequisites: None.
...
{% assign category = site.data.formula_catalog.categories | where: "name", "startup" | first %}
{% assign entries = site.data.formula_catalog.formulas | where: "category", "startup" %}

This page is generated from the docstrings of
[`vaft/formula/startup.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/formula/startup.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.formula.describe("startup.<name>")` prints.
