---
title: "Formula reference: atomic"
author: VEST team
date: 2026-09-02 10:14
category: guide
layout: post
permalink: /reference/formula/atomic/
guide:
  architecture: Generated per-function reference for vaft.formula.atomic, read from the standardized docstrings (issue 248).
  prerequisites: None.
...
{% assign category = site.data.formula_catalog.categories | where: "name", "atomic" | first %}
{% assign entries = site.data.formula_catalog.formulas | where: "category", "atomic" %}

This page is generated from the docstrings of
[`vaft/formula/atomic.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/formula/atomic.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.formula.describe("atomic.<name>")` prints.
