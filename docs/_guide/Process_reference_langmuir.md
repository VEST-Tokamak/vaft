---
title: "Process reference: langmuir"
author: VEST team
date: 2026-09-03 10:07
category: guide
layout: post
permalink: /reference/process/langmuir/
guide:
  architecture: Generated per-function reference for vaft.process.langmuir, read from the standardized docstrings (issue 252).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "langmuir" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "langmuir" %}

This page is generated from the docstrings of
[`vaft/process/langmuir.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/langmuir.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("langmuir.<name>")` prints.
