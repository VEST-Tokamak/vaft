---
title: "Process reference: magnetics"
author: VEST team
date: 2026-09-03 10:10
category: guide
layout: post
permalink: /reference/process/magnetics/
guide:
  architecture: Generated per-function reference for vaft.process.magnetics, read from the standardized docstrings (issue 252).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "magnetics" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "magnetics" %}

This page is generated from the docstrings of
[`vaft/process/magnetics.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/magnetics.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("magnetics.<name>")` prints.
