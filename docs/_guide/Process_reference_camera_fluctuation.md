...
guide:
  architecture: Generated per-function reference for vaft.process.camera_fluctuation, read from the standardized docstrings (issue 252).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "camera_fluctuation" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "camera_fluctuation" %}

This page is generated from the docstrings of
[`vaft/process/camera_fluctuation.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/camera_fluctuation.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("camera_fluctuation.<name>")` prints.
