...
guide:
  architecture: Generated per-function reference for vaft.process.cocos, read from the standardized docstrings (issue 252).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "cocos" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "cocos" %}

This page is generated from the docstrings of
[`vaft/process/cocos.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/cocos.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("cocos.<name>")` prints.
