---
title: "Process reference: camera_geometry"
author: VEST team
date: 2026-09-03 10:08
category: guide
layout: post
permalink: /reference/process/camera_geometry/
guide:
  architecture: Generated per-function reference for vaft.process.camera_geometry, read from the standardized docstrings (issue 252).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "camera_geometry" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "camera_geometry" %}

This page is generated from the docstrings of
[`vaft/process/camera_geometry.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/camera_geometry.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("camera_geometry.<name>")` prints.
