---
title: "Formula reference: virial"
author: VEST team
date: 2026-09-02 10:18
category: guide
layout: post
permalink: /reference/formula/virial/
guide:
  architecture: Generated per-function reference for vaft.formula.virial, read from the standardized docstrings (issue 248).
  prerequisites: None.
...
{% assign category = site.data.formula_catalog.categories | where: "name", "virial" | first %}
{% assign entries = site.data.formula_catalog.formulas | where: "category", "virial" %}

This page is generated from the docstrings of
[`vaft/formula/virial.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/formula/virial.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.formula.describe("virial.<name>")` prints.
