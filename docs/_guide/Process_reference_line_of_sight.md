...
guide:
  architecture: Generated per-function reference for vaft.process.line_of_sight, read from the standardized docstrings (issues 252 and 886).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "line_of_sight" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "line_of_sight" %}

This page is generated from the docstrings of
[`vaft/process/line_of_sight.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/line_of_sight.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("line_of_sight.<name>")` prints.
