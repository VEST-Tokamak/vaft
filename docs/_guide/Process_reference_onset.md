---
title: "Process reference: onset"
author: VEST team
date: 2026-09-07 09:00
category: guide
layout: post
permalink: /reference/process/onset/
guide:
  architecture: Generated per-function reference for vaft.process.onset, read from the standardized docstrings (issue 252).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "onset" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "onset" %}

This page is generated from the docstrings of
[`vaft/process/onset.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/onset.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("onset.<name>")` prints.
