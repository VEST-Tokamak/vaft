---
title: "Process reference: signal_processing"
author: VEST team
date: 2026-09-03 10:01
category: guide
layout: post
permalink: /reference/process/signal_processing/
guide:
  architecture: Generated per-function reference for vaft.process.signal_processing, read from the standardized docstrings (issue 252).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "signal_processing" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "signal_processing" %}

This page is generated from the docstrings of
[`vaft/process/signal_processing.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/signal_processing.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("signal_processing.<name>")` prints.
