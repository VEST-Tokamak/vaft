---
title: "Process reference: field_line_topology"
author: VEST team
date: 2026-09-20 12:00
category: guide
layout: post
permalink: /reference/process/field_line_topology/
guide:
  architecture: Generated per-function reference for vaft.process.field_line_topology, read from the standardized docstrings (issues 252 and 1099).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "field_line_topology" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "field_line_topology" %}

This page is generated from the docstrings of
[`vaft/process/field_line_topology.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/field_line_topology.py):
{{ entries.size }} public functions.  The category overview, notation and conventions come from the module
docstring; every entry below is what `vaft.process.describe("field_line_topology.<name>")` prints.
