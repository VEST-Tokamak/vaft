---
title: "Process reference: magnetic_island"
author: VEST team
date: 2026-09-18 09:00
category: guide
layout: post
permalink: /reference/process/magnetic_island/
guide:
  architecture: Generated per-function reference for vaft.process.magnetic_island, read from the standardized docstrings (issues 252 and 886).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "magnetic_island" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "magnetic_island" %}

This page is generated from the docstrings of
[`vaft/process/magnetic_island.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/magnetic_island.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("magnetic_island.<name>")` prints.
