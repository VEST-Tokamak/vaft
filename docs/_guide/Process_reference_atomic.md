---
title: "Process reference: atomic"
author: VEST team
date: 2026-09-03 10:05
category: guide
layout: post
permalink: /reference/process/atomic/
guide:
  architecture: Generated per-function reference for vaft.process.atomic, read from the standardized docstrings (issue 252).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "atomic" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "atomic" %}

This page is generated from the docstrings of
[`vaft/process/atomic.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/atomic.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("atomic.<name>")` prints.
