...
| [`analytic_island_model_and_synthetic_response_model.ipynb`](https://github.com/VEST-Tokamak/vaft/blob/{{ site.data.notebook_outputs.source_commit }}/notebooks/analytic_island_model_and_synthetic_response_model.ipynb) | Put an analytic m/n magnetic island on an equilibrium (Solov'ev and its CHEASE refinement) and compute the synthetic VEST soft X-ray chord response as it rotates. |
...
| [`fixed_boundary_parametric_scan_using_chease.ipynb`](https://github.com/VEST-Tokamak/vaft/blob/{{ site.data.notebook_outputs.source_commit }}/notebooks/fixed_boundary_parametric_scan_using_chease.ipynb) | Local fixed-boundary sensitivity of a CHEASE equilibrium: pressure, current-peaking and shape scans kept apart as requested, materialized and achieved, a pressure x peaking grid, a response matrix, and failed cases kept as results. |
...
| [`convergence_study_of_efit.ipynb`](https://github.com/VEST-Tokamak/vaft/blob/{{ site.data.notebook_outputs.source_commit }}/notebooks/convergence_study_of_efit.ipynb) | EFIT's Picard trajectory per slice (#1038): chi-square, flux increment and magnetic-axis height at every iteration, and `NXITER=1` against `NXITER=3`, read from saved iteration histories without running EFIT. |
