---
title: "Process reference: perturbation"
author: VEST team
date: 2026-09-14 12:00
category: guide
layout: post
permalink: /reference/process/perturbation/
guide:
  architecture: Generated per-function reference for vaft.process.perturbation, read from the standardized docstrings (issue 252).
  prerequisites: None.
...
{% assign category = site.data.process_catalog.categories | where: "name", "perturbation" | first %}
{% assign entries = site.data.process_catalog.functions | where: "category", "perturbation" %}

This page is generated from the docstrings of
[`vaft/process/perturbation.py`](https://github.com/VEST-Tokamak/vaft/blob/develop/vaft/process/perturbation.py):
{{ entries.size }} public functions.  The category overview and notation come from the module
docstring; every entry below is what `vaft.process.describe("perturbation.<name>")` prints.
