...
# =============================================================================
# Magnetic and Geometry Calculations
# =============================================================================
def calc_grid(
    xvar: List[float],
    zvar: List[float],
    ...
    :param pf: Dictionary describing active PF coil geometry and properties.
    :param pfp: Dictionary describing passive PF loop geometry and properties.
    ...
    for i, xr in enumerate(xvar):
        for j, zr in enumerate(zvar):
            ...
            for ii in range(nbloop):
                # geometry_type==1 => Outline polygon, geometry_type==2 => Rectangle
...
def vest_rspv1(
    ...
    """
    Compute the response matrix (Psi, Bz, Br) for a list of points in space.

    :param ods: OMAS data structure containing `pf_active` & `pf_passive`.
    :param plasma: List of [r, z] points for plasma elements (if any).
    :param rz: List of [r, z] observation points.
    :return: Tuple of (Psi, Bz, Br) arrays with shape (len(rz), nb_coil+nb_loop+nb_plasma).
    """
    pf = ods['pf_active']
    pfp = ods['pf_passive']
    ...
    br_array = np.zeros((num_points, nbcoil + nbloop + nbplas))
    bz_array = np.zeros((num_points, nbcoil + nbloop + nbplas))
    psi_array = np.zeros((num_points, nbcoil + nbloop + nbplas))

    for i, (r1, z1) in enumerate(rz):
        # From coils
        for ii in range(nbcoil):
            nbelti = len(pf[f'coil.{ii}.element'])
            sum_br, sum_bz, sum_psi = 0.0, 0.0, 0.0
            for jj in range(nbelti):
                nbturns = pf[f'coil.{ii}.element.{jj}.turns_with_sign']
                r2 = pf[f'coil.{ii}.element.{jj}.geometry.rectangle.r']
                z2 = pf[f'coil.{ii}.element.{jj}.geometry.rectangle.z']
                br_val, bz_val, p_val = compute_br_bz_phi(r1, z1, r2, z2)
                sum_br += br_val * nbturns
                sum_bz += bz_val * nbturns
                sum_psi += p_val * nbturns

            br_array[i][ii] = sum_br
            bz_array[i][ii] = sum_bz
            psi_array[i][ii] = sum_psi

        # From passive loops (wall)
        for ii in range(nbloop):
            # geometry_type==1 => Outline polygon, geometry_type==2 => Rectangle
            if pfp[f'loop.{ii}.element[0].geometry.geometry_type'] == 1:
                nbelti = len(
            ...
            br_val, bz_val, p_val = compute_br_bz_phi(r1, z1, r2, z2)
            index = nbcoil + ii
            br_array[i][index] = br_val
            bz_array[i][index] = bz_val
            psi_array[i][index] = p_val

        # From plasma
        for ii in range(nbplas):
            r2, z2 = plasma[ii]
            br_val, bz_val, p_val = compute_br_bz_phi(r1, z1, r2, z2)
            index = nbcoil + nbloop + ii
            br_array[i][index] = br_val
            bz_array[i][index] = bz_val
            psi_array[i][index] = p_val

    return psi_array, bz_array, br_array
