...
#   bash install/nubeam/windows.sh --nubeam-root PATH --accept-ntcc-terms
#
# Most people should run install/nubeam/windows.ps1 instead, which finds
...
usage() {
  cat <<'EOF'
Usage: bash install/nubeam/windows.sh --nubeam-root PATH --accept-ntcc-terms
...
[[ "$(uname -o 2>/dev/null)" == "Msys" ]] ||
  die "this installer runs inside MSYS2; start the UCRT64 shell, or use install/nubeam/windows.ps1"
[[ "${MSYSTEM:-}" == "UCRT64" ]] ||
...
if [[ -e "$ROOT_DIR/share/Make.local" ]] && ! grep -qE 'Generated by VAFT (install|external)/nubeam' "$ROOT_DIR/share/Make.local"; then
  die "refusing to overwrite an existing user Make.local: $ROOT_DIR/share/Make.local"
fi

mkdir -p "$BUILD_DIR" "$COMPAT_DIR" "$PREFIX"
# The manifest is what -Uninstall (windows.ps1) and uninstall.sh act on, in the
# format linux.sh writes. Kept across --resume, because the modules recorded
# by download_ntcc_module below are not downloaded a second time; an empty one
# is what VAFT 0.7.0 wrote and is filled in.
if [[ ! -s "$MANIFEST" ]]; then
  {
    printf 'root\t%s\n' "$ROOT_DIR"
...
# Generated by VAFT install/nubeam/windows.sh. Not part of the NTCC
...
download_ntcc_module() {
  ...
  mkdir -p "$destination"
  # Recorded before the copy, so an interrupted one is still ours to remove.
  # A module that was already there (the early return above) is never
  # recorded: the operator placed it, and -Uninstall must leave it.
