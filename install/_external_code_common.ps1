...
'@
    ...
    # or not ldd could be read, because ldd is the fragile half of this: it
    # runs the program to find out, so anything that stops the program
    # starting also stops ldd answering, and the result is an install that
    # reports nothing was needed and then fails its own load probe.
    $baseline = @(
        'libgcc_s_seh-1.dll',
        'libwinpthread-1.dll',
        'libgfortran-5.dll',
    ...
    foreach ($name in $wanted) {
        ...
        # ucrtbase -- is not ours to ship, and every name ldd reports that
        # does not live here is exactly one of those.
