...
function Copy-RuntimeDependencies {
    ...
    $unix = ConvertTo-Msys2Path -WindowsPath $BinDirectory
    $roots = @("/$MinGWEnvironment/bin")
    foreach ($extra in $ExtraSearchDirectories) {
        $roots += (ConvertTo-Msys2Path -WindowsPath $extra)
    }
    $pattern = ($roots | ForEach-Object { [regex]::Escape($_) }) -join '|'
    $command = "for e in '$unix'/*.exe; do ldd " + '"$e"' + "; done 2>/dev/null | grep -oiE '($pattern)/[A-Za-z0-9_.+-]+[.]dll' | sort -u"

    $bash = Join-Path $Msys2Root 'usr\bin\bash.exe'
    $saved = [Environment]::GetEnvironmentVariable('MSYSTEM', 'Process')
    [Environment]::SetEnvironmentVariable('MSYSTEM', $MinGWEnvironment.ToUpper(), 'Process')
    $previous = $ErrorActionPreference
    $ErrorActionPreference = 'Continue'
    try {
        ...
        [Environment]::SetEnvironmentVariable('MSYSTEM', $saved, 'Process')
    }
    ...
    foreach ($line in @($lines)) {
        ...
        $windowsSource = $null
        if ($text -like "/$MinGWEnvironment/bin/*") {
            $windowsSource = Join-Path $Msys2Root ($MinGWEnvironment + '\bin\' + $name)
        }
        else {
            foreach ($extra in $ExtraSearchDirectories) {
                $probe = Join-Path $extra $name
                if (Test-Path -LiteralPath $probe) { $windowsSource = $probe; break }
            }
        }
        if ($windowsSource -and (Test-Path -LiteralPath $windowsSource)) {
            $destination = Join-Path $BinDirectory $name
            if ((Resolve-Path -LiteralPath $windowsSource).Path -ne $destination) {
                Copy-Item -LiteralPath $windowsSource -Destination $destination -Force
            }
            $copied++
        }
    }
    if ($copied -eq 0) {
        Write-Result -Status SKIP -Name 'Runtime libraries' -Detail 'nothing beyond the system set is required'
    }
    else {
        Write-Result -Status PASS -Name 'Runtime libraries' -Detail "$copied library file(s) copied next to the executables"
    }
    return $copied
}
