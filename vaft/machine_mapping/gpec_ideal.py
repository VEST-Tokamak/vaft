...
def _write_flux_surface_geometry(entry: ODS, profile, jacobian: str) -> Optional[dict[str, Any]]:
    ...
    *different* ``psi_n[k]`` draws the right number in the wrong place.  The
    cost is measured: 6.4 MB per array on the DIII-D reference (513 poloidal
    x 1561 radial), so 12.8 MB per mode beside the 3.2 MB of spectral field.
    ``include_geometry=False`` turns it off for a caller that only wants the
    spectrum.
    """
    ...
    *,
    ...
    if (include_spectral or include_geometry) and "profile" in result.source_paths:
        ...
        if profile is not None:
            jacobian = control.jacobian or "hamada"
            if include_spectral:
...
def gpec_ideal(ods: ODS, source: str, options: Optional[dict] = None) -> dict[int, dict[str, Any]]:
    ...
    - ``include_geometry`` writes the flux-surface mesh
      ``R(psi_N, theta)`` / ``z(psi_N, theta)`` into
      ``plasma.coordinate_system``, which is what makes a real-space figure
      of the result drawable. It costs 12.8 MB per mode on the DIII-D
      reference and it opens the same profile file ``include_spectral`` does,
      which is why it defaults to whatever ``include_spectral`` is rather than
      to True: turning the spectral field off is how a caller says "do not
      open the 144 MB file", and the mesh is in that file. Pass it explicitly
    ...
    # Defaults to `include_spectral` rather than to True: the mesh lives in
    # the same profile file the spectral field does, and a caller who turned
    # that off did so to avoid opening a 144 MB file. Asking for the geometry
    # explicitly opens it.
    include_geometry = bool(options.get("include_geometry", include_spectral))
