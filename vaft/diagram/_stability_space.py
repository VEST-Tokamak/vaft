...
def _star(center, outer=0.24, inner=0.1) -> np.ndarray:
...
def _clip(xy: np.ndarray, chart: Chart) -> List[np.ndarray]:
...
def _render_chart(chart: Chart, *, x_label: str, y_label: str, curve_styles: Dict[str, str],
    ...
    for name, style in curve_styles.items():
        for run in _clip(chart.curves[name], chart):
            items.append(Polyline.of(chart.to_cm(run), style, role=name))
    if star:
        items.append(Polyline.of(_star(chart.to_cm(chart.points[star])), "star", role=star, closed=True))
