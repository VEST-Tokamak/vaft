...
def percentile_scale(values: np.ndarray | Iterable[float], percentile: float = 90.0) -> float:
    r"""A high percentile of ``|x|`` over the finite entries, an amplitude scale one anomalous sample cannot set.
    
    $$s_q = \operatorname{percentile}_q(|x_i|)$$
    
    Parameters
    ----------
    values : ndarray or iterable of float
        Samples; non-finite entries are ignored [units of values].
    percentile : float
    ...
    Assumptions
    -----------
    None.  It is not a noise estimator (see :func:`median_absolute_deviation`);
    ...
    References
    ----------
    .. [1] R. J. Hyndman and Y. Fan, Am. Stat. 50 (1996) 361 (sample quantile
           definitions; NumPy's default is their type 7).
    """
    finite = _finite(values)
    if not finite.size:
        return float("nan")
