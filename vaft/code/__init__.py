"""Lazy namespace for external fusion-code adapters."""

from importlib import import_module

__all__ = [
    "CodeConfig",
    "CodeInputs",
    "CodeResult",
    "CodeRunner",
    "ExecutableNotLaunchable",
    "ExecutionBackend",
    "ExecutionRequest",
    "ExecutionResult",
    "LocalBackend",
    "MemoryBudget",
    "MemoryBudgetExceeded",
    "MemoryBudgetInfo",
    "memory_budget",
    "peak_rss_mb",
    "rss_mb",
    "ResourceRequest",
    "resolve_backend",
    "SlurmBackend",
    "BoundaryContourPolicy",
    "CHEASEConfig",
    "CHEASEInputs",
    "CHEASEResult",
    "EFITConfig",
    "EFITConstraintConfig",
    "EFIT_FAILURE_CODES",
    "EFITInitializationConfig",
    "EFITInputs",
    "EFITNumericsConfig",
    "EFITProfileConfig",
    "EFITResult",
    "EFITScientificConfig",
    "EFITSliceStatus",
    "EFITValidationConfig",
    "GPECCaseInputs",
    "GPECModuleRun",
    "GPECSuiteConfig",
    "GPECSuiteResult",
    "NUBEAMConfig",
    "NUBEAMInputs",
    "NUBEAMResult",
    "base",
    "apply_temporal_continuity",
    "chease",
    "nubeam",
    "collect_efit_outputs",
    "collect_gpec_suite_outputs",
    "efit",
    "efit_parameter_grid",
    "gacode",
    "GACODEConfig",
    "find_gacode_executable",
    "gacode_environment",
    "gacode_home",
    "gacode_platform",
    "format_gfile_header_for_gpec",
    "CHEASEScanCase",
    "EquilibriumVariation",
    "apply_equilibrium_variation",
    "scan_chease",
    "find_chease_executable",
    "find_nubeam_executable",
    "generate_constraints_ods",
    "generate_kfile",
    "gfile_to_omas",
    "gpec",
    "init_snakemake_logger",
    "prepare_efit_inputs",
    "resolved_efit_configuration",
    "collect_nubeam_outputs",
    "prepare_chease_inputs",
    "prepare_nubeam_inputs",
    "prepare_gpec_suite_case",
    "refine_equilibrium",
    "run_efit",
    "validate_efit_slice",
    "run_chease",
    "run_nubeam",
    "run_nubeam_case",
    "run_gpec_suite_case",
    "snakemake",
    "tes",
    "TESConfig",
    "TESInputs",
    "TESResult",
    "prepare_tes_inputs",
    "run_tes",
    "collect_tes_outputs",
    "parse_result_scalars",
    "parse_result_coils",
    "scan_tes",
    "pentrc",
    "PentrcFormatError",
    "PentrcOutput",
    "read_pentrc_output",
    "tokamaker",
    "transp",
    "TRANSPResult",
    "TranspFormatError",
    "TranspOutput",
    "TranspSlice",
    "TranspVariable",
    "collect_transp_outputs",
    "exb_frequency",
    "kinetic_profiles_from_slice",
    "enclosed_torque",
    "input_torque_density",
    "read_transp_output",
    "read_transp_profiles",
    "zone_volume",
    "TokaMakerConfig",
    "TokaMakerInputs",
    "TokaMakerResult",
    "tokamaker_geometry_from_ods",
    "geometry_signature",
    "prepare_tokamaker_inputs",
    "resolve_mesh_file",
    "build_tokamaker_mesh",
    "run_tokamaker",
    "collect_tokamaker_outputs",
    "parse_stats_sidecar",
    "scan_tokamaker",
    "ScanTopology",
    "TopologyReport",
    "classify_boundary",
    "CaseStatus",
    "FreeBoundaryScan",
    "FreeBoundaryScanResult",
    "FreeBoundaryCaseResult",
    "free_boundary_scan",
    "TokaMakerEvolutionInputs",
    "TokaMakerEvolutionResult",
    "TokaMakerStepRecord",
    "TokaMakerStabilityResult",
    "vessel_segments_from_ods",
    "prepare_tokamaker_evolution_inputs",
    "run_tokamaker_evolution",
    "run_tokamaker_wall_eigenmodes",
    "run_tokamaker_vertical_stability",
    "collect_tokamaker_evolution_outputs",
    "collect_tokamaker_stability_outputs",
    "KineticEFITConfig",
    "KineticEFITInputs",
    "KineticEFITResult",
    "NiceConfig",
    "NiceDiagnostic",
    "NiceInputs",
    "NiceResult",
    "PressurePoints",
    "build_kinetic_core_profiles",
    "kinetic_pressure_points",
    "inject_pressure_constraint",
    "scale_plasma",
    "prepare_kinetic_efit_inputs",
    "run_kinetic_efit",
    "run_kinetic_chain",
    "prepare_nice_inputs",
    "run_nice",
    "collect_nice_outputs",
    "nice_geometry_from_ods",
    "diagnostics_from_ods",
    "vest_reference_parameter_file",
]

_EXPORT_MAP = {
    "GACODEConfig": (".gacode", "GACODEConfig"),
    "find_gacode_executable": (".gacode", "find_gacode_executable"),
    "gacode_environment": (".gacode", "gacode_environment"),
    "gacode_home": (".gacode", "gacode_home"),
    "gacode_platform": (".gacode", "gacode_platform"),
    "CodeConfig": (".base", "CodeConfig"),
    "CodeInputs": (".base", "CodeInputs"),
    "CodeResult": (".base", "CodeResult"),
    "CodeRunner": (".base", "CodeRunner"),
    "ExecutableNotLaunchable": ("._executables", "ExecutableNotLaunchable"),
    "ExecutionBackend": (".execution", "ExecutionBackend"),
    "ExecutionRequest": (".execution", "ExecutionRequest"),
    "ExecutionResult": (".execution", "ExecutionResult"),
    "LocalBackend": (".execution", "LocalBackend"),
    "MemoryBudget": (".resources", "MemoryBudget"),
    "MemoryBudgetExceeded": (".resources", "MemoryBudgetExceeded"),
    "MemoryBudgetInfo": (".resources", "MemoryBudgetInfo"),
    "memory_budget": (".resources", "memory_budget"),
    "peak_rss_mb": (".resources", "peak_rss_mb"),
    "rss_mb": (".resources", "rss_mb"),
    "ResourceRequest": (".execution", "ResourceRequest"),
    "resolve_backend": (".execution", "resolve_backend"),
    "SlurmBackend": (".slurm", "SlurmBackend"),
    "BoundaryContourPolicy": (".chease", "BoundaryContourPolicy"),
    "CHEASEConfig": (".chease", "CHEASEConfig"),
    "CHEASEInputs": (".chease", "CHEASEInputs"),
    "CHEASEResult": (".chease", "CHEASEResult"),
    "NUBEAMConfig": (".nubeam", "NUBEAMConfig"),
    "NUBEAMInputs": (".nubeam", "NUBEAMInputs"),
    "NUBEAMResult": (".nubeam", "NUBEAMResult"),
    "EFITConfig": (".efit", "EFITConfig"),
    "EFITConstraintConfig": (".efit", "EFITConstraintConfig"),
    "EFIT_FAILURE_CODES": (".efit", "EFIT_FAILURE_CODES"),
    "EFITInputs": (".efit", "EFITInputs"),
    "EFITInitializationConfig": (".efit", "EFITInitializationConfig"),
    "EFITNumericsConfig": (".efit", "EFITNumericsConfig"),
    "EFITProfileConfig": (".efit", "EFITProfileConfig"),
    "EFITResult": (".efit", "EFITResult"),
    "EFITScientificConfig": (".efit", "EFITScientificConfig"),
    "EFITSliceStatus": (".efit", "EFITSliceStatus"),
    "EFITValidationConfig": (".efit", "EFITValidationConfig"),
    "GPECCaseInputs": (".gpec", "GPECCaseInputs"),
    "GPECModuleRun": (".gpec", "GPECModuleRun"),
    "GPECSuiteConfig": (".gpec", "GPECSuiteConfig"),
    "GPECSuiteResult": (".gpec", "GPECSuiteResult"),
    "find_chease_executable": (".chease", "find_chease_executable"),
    "CHEASEScanCase": (".chease_scan", "CHEASEScanCase"),
    "EquilibriumVariation": (".chease_scan", "EquilibriumVariation"),
    "apply_equilibrium_variation": (".chease_scan", "apply_equilibrium_variation"),
    "scan_chease": (".chease_scan", "scan_chease"),
    "find_nubeam_executable": (".nubeam", "find_nubeam_executable"),
    "collect_nubeam_outputs": (".nubeam", "collect_nubeam_outputs"),
    "apply_temporal_continuity": (".efit", "apply_temporal_continuity"),
    "collect_efit_outputs": (".efit", "collect_efit_outputs"),
    "collect_gpec_suite_outputs": (".gpec", "collect_gpec_suite_outputs"),
    "format_gfile_header_for_gpec": (".gpec", "format_gfile_header_for_gpec"),
    "efit_parameter_grid": (".efit", "efit_parameter_grid"),
    "generate_constraints_ods": (".efit", "generate_constraints_ods"),
    "generate_kfile": (".efit", "generate_kfile"),
    "gfile_to_omas": (".efit", "gfile_to_omas"),
    "init_snakemake_logger": (".snakemake", "init_snakemake_logger"),
    "prepare_chease_inputs": (".chease", "prepare_chease_inputs"),
    "prepare_nubeam_inputs": (".nubeam", "prepare_nubeam_inputs"),
    "prepare_efit_inputs": (".efit", "prepare_efit_inputs"),
    "resolved_efit_configuration": (".efit", "resolved_efit_configuration"),
    "prepare_gpec_suite_case": (".gpec", "prepare_gpec_suite_case"),
    "refine_equilibrium": (".chease", "refine_equilibrium"),
    "run_efit": (".efit", "run_efit"),
    "validate_efit_slice": (".efit", "validate_efit_slice"),
    "run_chease": (".chease", "run_chease"),
    "run_nubeam": (".nubeam", "run_nubeam"),
    "run_nubeam_case": (".nubeam", "run_nubeam_case"),
    "run_gpec_suite_case": (".gpec", "run_gpec_suite_case"),
    "TESConfig": (".tes", "TESConfig"),
    "TESInputs": (".tes", "TESInputs"),
    "TESResult": (".tes", "TESResult"),
    "prepare_tes_inputs": (".tes", "prepare_tes_inputs"),
    "run_tes": (".tes", "run_tes"),
    "collect_tes_outputs": (".tes", "collect_tes_outputs"),
    "parse_result_scalars": (".tes", "parse_result_scalars"),
    "parse_result_coils": (".tes", "parse_result_coils"),
    "scan_tes": (".tes", "scan_tes"),
    "PentrcFormatError": (".pentrc", "PentrcFormatError"),
    "PentrcOutput": (".pentrc", "PentrcOutput"),
    "read_pentrc_output": (".pentrc", "read_pentrc_output"),
    "TRANSPResult": (".transp", "TRANSPResult"),
    "TranspFormatError": (".transp", "TranspFormatError"),
    "TranspOutput": (".transp", "TranspOutput"),
    "TranspSlice": (".transp", "TranspSlice"),
    "TranspVariable": (".transp", "TranspVariable"),
    "collect_transp_outputs": (".transp", "collect_transp_outputs"),
    "enclosed_torque": (".transp", "enclosed_torque"),
    "input_torque_density": (".transp", "input_torque_density"),
    "read_transp_output": (".transp", "read_transp_output"),
    "read_transp_profiles": (".transp", "read_transp_profiles"),
    "kinetic_profiles_from_slice": (".transp", "kinetic_profiles_from_slice"),
    "exb_frequency": (".transp", "exb_frequency"),
    "zone_volume": (".transp", "zone_volume"),
    "TokaMakerConfig": (".tokamaker", "TokaMakerConfig"),
    "TokaMakerInputs": (".tokamaker", "TokaMakerInputs"),
    "TokaMakerResult": (".tokamaker", "TokaMakerResult"),
    "tokamaker_geometry_from_ods": (".tokamaker", "tokamaker_geometry_from_ods"),
    "geometry_signature": (".tokamaker", "geometry_signature"),
    "prepare_tokamaker_inputs": (".tokamaker", "prepare_tokamaker_inputs"),
    "resolve_mesh_file": (".tokamaker", "resolve_mesh_file"),
    "build_tokamaker_mesh": (".tokamaker", "build_tokamaker_mesh"),
    "run_tokamaker": (".tokamaker", "run_tokamaker"),
    "collect_tokamaker_outputs": (".tokamaker", "collect_tokamaker_outputs"),
    "parse_stats_sidecar": (".tokamaker", "parse_stats_sidecar"),
    "scan_tokamaker": (".tokamaker", "scan_tokamaker"),
    "ScanTopology": (".tokamaker", "ScanTopology"),
    "TopologyReport": (".tokamaker", "TopologyReport"),
    "classify_boundary": (".tokamaker", "classify_boundary"),
    "CaseStatus": (".tokamaker", "CaseStatus"),
    "FreeBoundaryScan": (".tokamaker", "FreeBoundaryScan"),
    "FreeBoundaryScanResult": (".tokamaker", "FreeBoundaryScanResult"),
    "FreeBoundaryCaseResult": (".tokamaker", "FreeBoundaryCaseResult"),
    "free_boundary_scan": (".tokamaker", "free_boundary_scan"),
    "TokaMakerEvolutionInputs": (".tokamaker", "TokaMakerEvolutionInputs"),
    "TokaMakerEvolutionResult": (".tokamaker", "TokaMakerEvolutionResult"),
    "TokaMakerStepRecord": (".tokamaker", "TokaMakerStepRecord"),
    "TokaMakerStabilityResult": (".tokamaker", "TokaMakerStabilityResult"),
    "vessel_segments_from_ods": (".tokamaker", "vessel_segments_from_ods"),
    "prepare_tokamaker_evolution_inputs": (".tokamaker", "prepare_tokamaker_evolution_inputs"),
    "run_tokamaker_evolution": (".tokamaker", "run_tokamaker_evolution"),
    "run_tokamaker_wall_eigenmodes": (".tokamaker", "run_tokamaker_wall_eigenmodes"),
    "run_tokamaker_vertical_stability": (".tokamaker", "run_tokamaker_vertical_stability"),
    "collect_tokamaker_evolution_outputs": (".tokamaker", "collect_tokamaker_evolution_outputs"),
    "collect_tokamaker_stability_outputs": (".tokamaker", "collect_tokamaker_stability_outputs"),
    "KineticEFITConfig": (".efit", "KineticEFITConfig"),
    "KineticEFITInputs": (".efit", "KineticEFITInputs"),
    "KineticEFITResult": (".efit", "KineticEFITResult"),
    "NiceConfig": (".nice", "NiceConfig"),
    "NiceDiagnostic": (".nice", "NiceDiagnostic"),
    "NiceInputs": (".nice", "NiceInputs"),
    "NiceResult": (".nice", "NiceResult"),
    "PressurePoints": (".efit", "PressurePoints"),
    "build_kinetic_core_profiles": (".efit", "build_kinetic_core_profiles"),
    "kinetic_pressure_points": (".efit", "kinetic_pressure_points"),
    "inject_pressure_constraint": (".efit", "inject_pressure_constraint"),
    "scale_plasma": (".efit", "scale_plasma"),
    "prepare_kinetic_efit_inputs": (".efit", "prepare_kinetic_efit_inputs"),
    "run_kinetic_efit": (".efit", "run_kinetic_efit"),
    "run_kinetic_chain": (".efit", "run_kinetic_chain"),
    "prepare_nice_inputs": (".nice", "prepare_nice_inputs"),
    "run_nice": (".nice", "run_nice"),
    "collect_nice_outputs": (".nice", "collect_nice_outputs"),
    "nice_geometry_from_ods": (".nice", "nice_geometry_from_ods"),
    "diagnostics_from_ods": (".nice", "diagnostics_from_ods"),
    "vest_reference_parameter_file": (".nice", "vest_reference_parameter_file"),
}


def __getattr__(name: str):
    if name in {
        "base",
        "efit",
        "execution",
        "gacode",
        "resources",
        "slurm",
        "gpec",
        "chease",
        "nubeam",
        "snakemake",
        "tes",
        "tokamaker",
        "transp",
        "pentrc",
        "nice",
    }:
        module = import_module(f".{name}", __name__)
        globals()[name] = module
        return module
    if name not in _EXPORT_MAP:
        raise AttributeError(f"module {__name__!r} has no attribute {name!r}")
    module_name, attribute = _EXPORT_MAP[name]
    module = import_module(module_name, __name__)
    value = getattr(module, attribute)
    globals()[name] = value
    return value


def __dir__():
    return sorted(list(globals().keys()) + __all__)
