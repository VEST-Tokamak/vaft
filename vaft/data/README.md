...
| `samples/41524/` | `manifest.yaml`, `imas.nc` | Complete repository-only pipeline example normalized to native IMAS DD 3.41 |
...
representation is maintained independently. Shot 41524 is regenerated by
`generate_legacy_imas_sample.py` from its legacy JSON blob pinned in Git
history. Shot 41672 is regenerated from its frozen current-pipeline product by
`generate_pipeline_imas_sample.py`; it retains the successful EFIT slices and
records unavailable optional channels and unsuccessful EFIT times in its
