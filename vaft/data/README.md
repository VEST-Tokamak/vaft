...
| `samples/{41524,41672}/` | `manifest.yaml`, `imas.nc` | Complete repository-only pipeline examples normalized to native IMAS DD 3.41 |
...
regenerated by `generate_legacy_imas_sample.py` from legacy JSON blobs pinned
in Git history. Their conversion preserves scientific arrays while adding
required coupling coordinates, removing non-IMAS EFIT cache metadata, and
correcting the probe direction metadata to +Bz. The old format-grouped JSON
files and unrelated IMAS NetCDF sample are intentionally not recreated.
