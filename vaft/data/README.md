...
| EFIT inputs that are not Green tables | `lim.dat`, `dprobe.dat`, `rfcoil.txt` | `lim.dat` is the limiter outline EFIT reads from `TABLE_DIR`; it must equal the wall outline in `VEST_static_geometry.json.gz`, whose inboard face is at R = 0.105 m so that no EFIT grid column sits on it (#965, guarded by `test/test_limiter_grid_clearance.py`). `dprobe.dat` is not read by this EFIT. `rfcoil.txt` is a text rendering kept for reference. |
